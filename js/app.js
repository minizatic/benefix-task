$(function(){
	...
	function extractDataFromFiles(files) {
		return $.when.apply($, Array.from(files).map(extractDataFromPDF));
	}
		...
		return new $.Deferred(function(def) {
			...
			pdfReader.onload = function(){
				...
				PDFJS.getDocument(pdfData).then(function(pdf){
					...
					def.resolve(pageDataArray);
				}, function(err){
					console.log(err);
				});
			};
			pdfReader.onerror = pdfReader.onabort = def.reject;
	...
	function processFiles(event) {
		...
		reader.onload = function(){
			...
			extractDataFromFiles(pdfFiles).then(function(results){
				...
				console.log(results);
				console.log(ws);
				XLSX.utils.book_append_sheet(wb, ws, "FancyName");
